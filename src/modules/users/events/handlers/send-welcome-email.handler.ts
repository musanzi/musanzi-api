  ...
  async handle(event: WelcomeUserEvent): Promise<void> {
    try {
      const content = buildEmailBody({
        title: 'Bienvenue sur musanzi',
        greetingName: event.user.name,
        intro: 'Votre compte a bien été créé. Vous pouvez maintenant vous connecter et commencer à utiliser musanzi.',
